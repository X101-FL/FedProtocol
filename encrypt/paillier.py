...
from .tools.fixedpoint import FixedPointNumber
...
class PaillierKeypair:
    ...
    @staticmethod
    def generate_keypair(n_length=1024, precision=None):
        ...
        public_key = PaillierPublicKey(n, precision)
...
class PaillierPublicKey:
    ...
    def __init__(self, n, precision):
        ...
        self.nsquare = n * n
        self.max_int = n // 3 - 1
        self.encode_exponent = precision and -math.floor(math.log(precision, FixedPointNumber.BASE))
    ...
    def encrypt(self, value, random_value=None):
        """Encode and Paillier encrypt a real number value.
        """
        encoding = FixedPointNumber.encode(value,
                                           self.n,
                                           self.max_int,
                                           min(FixedPointNumber.lossless_exponent(value), self.encode_exponent))
...
class PaillierEncryptedNumber:
    ...
    def __repr__(self):
        return f"exponent={self.exponent}, hash(cipher)={hash(self.__ciphertext)}"
    ...
    def __mul__(self, scalar):
        ...
        encode = FixedPointNumber.encode(scalar,
                                         self.public_key.n,
                                         self.public_key.max_int,
                                         min(FixedPointNumber.lossless_exponent(scalar),
                                             self.public_key.encode_exponent))
    ...
    def __add_scalar(self, scalar):
        ...
        encoded = FixedPointNumber.encode(scalar,
                                          self.public_key.n,
                                          self.public_key.max_int,
                                          max(self.public_key.encode_exponent, self.exponent))
