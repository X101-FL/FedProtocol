...
class BaseSender(BaseWorker):
    ...
    def run(self, m0: bytes, m1: bytes, curve: str = "secp256r1") -> None:
...
class BaseReceiver(BaseWorker):
    ...
    def run(self, b: int) -> bytes:
        ...
        self.comm.send(
            receiver="BaseSender", message_name="public_key", obj=key_to_bytes(pk)
        )

        ck = ak.pointQ * sk.d
        # receive cipher message
        cipher_m = unpack(
            self.comm.receive(sender="BaseSender", message_name="cipher_message")
        )[b]
