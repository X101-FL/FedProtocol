...
class PSIServer(BaseClient):
    ...
    def init(self):
        self.set_sub_client(self.oprf_server, role_rename_dict={"OPRFServer": "PSIServer",
    ...
    def run(self):
        ...
        for i in range(3):
            ...
            for word in words:
                ...
                self.logger.debug(f"hash index {i + 1}, table position {h},"
        ...
        for i in range(self.s):
            ...
            for word in words:
                ...
                self.logger.debug(f"table position {n + i}, "
                                  f"word {int.from_bytes(word, 'big')}, val {val.hex()}")
                self.comm.send(receiver="PSIClient", message_name="word", obj=val)
            self.comm.send(receiver="PSIClient", message_name="word", obj=b"end")
    ...
    def run(self):
        ...
        for i, (key, hash_index) in enumerate(zip(self.table, self.table_hash_index)):
            if key is not None:
                ...
                self.logger.debug(f"hash index {hash_index}, table position {i}, "
