...
class LocalComm(BaseComm):
    ...
    def _send(
        ...
        for message_name, message_obj in message_package:
            self.logger.debug(
                f"Put a message: message_space={self.message_space}, sender={self.role_name}, "
                f"receiver={receiver}, message_name={message_name}"
            )
            self.message_space_manager.put(
                self.role_name, receiver, message_name, message_obj
            )
    ...
    def receive(
        ...
        self.logger.debug(
            f"Get a message: message_space={self.message_space}, sender={sender}, "
            f"receiver={self.role_name}, message_name={message_name}"
        )
        message_queue = self.message_space_manager.get_message_queue(
            sender, self.role_name, message_name
        )
    ...
    def watch_(
        ...
        self.logger.debug(
            f"Register watch: message_space={self.message_space}, receiver={self.role_name}"
        )
        watch_manager = self.message_space_manager.register_watch(
            self.role_name, sender_message_name_tuple_list
        )
    ...
    def clear(
        ...
        self.logger.debug(
            f"clear cached messages : message_space={self.message_space}, sender={sender}, "
            f"receiver={self.role_name}, message_name={message_name}"
        )
        drop_size = 0
        message_queues = self.message_space_manager.lookup_message_queues(
            sender, self.role_name, message_name
        )
        for message_id, message_queue in message_queues:
    ...
    def _sub_comm(
        ...
        return LocalComm(
            message_space=f"{self.message_space}.{protocol_name}",
            role_name=role_name,
    ...
    def _get_message(
        self, message_queue: Union[Queue, WatchManager], timeout: Optional[int]
    ...
    def _assert_role_name(
        self, sender: Optional[Sender] = None, receiver: Optional[Receiver] = None
    ):
        assert (not sender) or (
            sender in self.other_role_name_set
        ), f"Unknown sender: {sender}, acceptable senders are:  {self.other_role_name_set}"
        assert (not receiver) or (
