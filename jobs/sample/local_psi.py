...
class ActiveClient(BaseClient):
    ...
    def __init__(self):
        ...
        self.psi_server = PSIServer(words)
    ...
    def run(self):
        ...
        self.logger.debug(f"PSIServer prepare complete")

        self.logger.debug(f"Start intersection")
        res = self.psi_server.run()  # intersect stage, res is the intersection
        self.logger.debug(f"Finish intersection")
        ans = sorted([int.from_bytes(val, "big") for val in res])
        self.logger.info(f"result is {ans}")
...
class PassiveClient(BaseClient):
    ...
    def __init__(self):
        ...
        self.psi_client = PSIClient(words)
    ...
    def run(self):
        ...
        self.logger.debug(f"PSIClient prepare complete")

        self.logger.debug(f"Start intersection")
        res = self.psi_client.run()  # intersect stage, res is the intersection
        self.logger.debug(f"Finish intersection")
        ans = sorted([int.from_bytes(val, "big") for val in res])
        self.logger.info(f"Results is {ans}")
